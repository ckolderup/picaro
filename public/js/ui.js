...
define(["jquery", "game", "util", "item", "room", "inventory", "talk", "vendor/underscore"], function($, Game, util, Item, Room, Inventory, Talk) {
  var UI, itemTriggers;
  itemTriggers = [];
  UI = {
    messageDiv: $("#game"),
    ...
    renderNounMenuItem: function(item, verb, showHeld) {
      var heldIndicator;
    ...
    renderVerbMenuItem: function(item, verb) {
      return "<li>        <a href='#' class='verb action' data-action-id='" + (util.actionId(item, verb)) + "'>          " + verb + "        </a>      </li>";
    },
    renderInlineMenuItem: function() {
      var item, listItems, verb, _i, _j, _len, _len1, _ref, _ref1;
      item = Item.find($(this).data('item-id'));
      listItems = '';
      _ref = Game.current.unaryVerbs;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        verb = _ref[_i];
        if (item[verb]) {
          listItems += UI.renderVerbMenuItem(item, verb);
        }
      }
      _ref1 = Game.current.binaryVerbs;
    ...
    resetMenus: function() {
      var item, itemsForMenus, self, verb, _i, _j, _len, _len1, _ref, _results;
      ...
      for (_i = 0, _len = itemsForMenus.length; _i < _len; _i++) {
        ...
        _results.push((function() {
          ...
          for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
            verb = _ref1[_k];
            _results1.push(UI.renderNounMenuItem(item, verb, Inventory.include(item.id)));
          }
          return _results1;
        })());
      }
    ...
    itemAction: function(action, item, event) {
      item = Item.find(item);
      if (action === "look") {
    ...
    init: function(gameName) {
      ...
      oldMenus.each(function(action) {
        ...
        return $(menuSelector + " ul li a").live("click", function(event) {
          ...
          return UI.itemAction(actionAndId[0], actionAndId[1], event);
        });
      });
      ...
      return $("#move-compass li:not(.disabled) a").mouseout(function() {
  ...
  $('.verb.action').live('click', function(event) {
    var actionAndId;
    actionAndId = util.splitActionId(this);
    $(this).trigger("closeMenu");
    return UI.itemAction(actionAndId[0], actionAndId[1], event);
